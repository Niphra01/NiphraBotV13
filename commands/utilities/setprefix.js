...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (args[0].length > 1) return message.reply({ content: "**Please provide single digit Letter/Symbol/Number**" })
        ...
        if (findResult.some((item) => item.GuildID.includes(getGuildID))) {
            await Mongo.dbo.collection("BotEnv").updateOne({ GuildID: getGuildID }, { $set: { GuildPrefix: PREFIX } });
